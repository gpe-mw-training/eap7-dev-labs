...
public abstract class PerfBase {
   ...
   private void init() throws Exception {
      if (perfParams.isOpenwire()) {
         PerfBase.log.info("OPEN WIRE not supported");
         ...
         environment.put(Context.PROVIDER_URL, perfParams.getUri());
         InitialContext initialContext = new InitialContext(environment);

         destination = (Destination)initialContext.lookup(perfParams.getDestinationName());

         factory = (ConnectionFactory) initialContext.lookup("jms/RemoteConnectionFactory");
         connection = factory.createConnection(userName, password);

      } else if (perfParams.isAMQP()) {
         PerfBase.log.info("AMQP not supported");
      }
